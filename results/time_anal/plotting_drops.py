...
# Identify drops
spike_threshold = 0.5  # Threshold ratio
dataset_lookup = 20
spike_days = []
...
for i in range(dataset_lookup, len(reviews_per_day) - dataset_lookup):
    ...
    if current_day != 0 and current_day < prev_day * spike_threshold and current_day < next_day * spike_threshold:
...
for day in spike_days:
    ...
    plt.title(f"Drop on {day.strftime('%Y-%m-%d')}")
    plt.xlabel('Date')
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'figures/drops/drop_{day.strftime("%Y-%m-%d")}.png')
